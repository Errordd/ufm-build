...
class PlayState extends MusicBeatState
{
	...
	public var healthBarFillP1:FlxSprite; // Player's fill
	public var healthBarFillP2:FlxSprite; // Opponent's fill
	public var maskHealthBar:FlxSprite;
	public var timeBar:Bar;
	var songPercent:Float = 0;

	private var timeTxtTween:FlxTween = null;
	private var lastTimerBopDirection:Int = 0; // 0 = horizontal, 1 = vertical or whatever idfk
	...
	public function doTimerBop():Void {
    	...
    	lastTimerBopDirection = (lastTimerBopDirection == 0) ? 1 : 0;
    
    	if (lastTimerBopDirection == 0) {
        // Horizontal squeeze - more subtle
        	timeTxt.scale.x = 1.08;
        	timeTxt.scale.y = 0.97;
    	} else {
        	timeTxt.scale.x = 0.97;
        	timeTxt.scale.y = 1.08;
    	}
    
    	timeTxtTween = FlxTween.tween(timeTxt.scale, {x: 1, y: 1}, 0.6, {
	...
	public function setupHealthBar() {
    	var barY = FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11) - 30; // Move up by 30 pixels

		healthBarBG = new FlxSprite(0, barY);
		healthBarBG.loadGraphic(Paths.image('healthBar'));
		healthBarBG.screenCenter(X);

		healthBarFill = new FlxSprite(healthBarBG.x, healthBarBG.y);
		healthBarFill.loadGraphic(Paths.image('healthbarFill'));
		healthBarFill.setGraphicSize(Std.int(healthBarBG.width * 1.01), Std.int(healthBarBG.height * 1.03));
		healthBarFill.updateHitbox();
		healthBarFill.x = healthBarBG.x;
		healthBarFill.x -= 2;
		healthBarFill.y = healthBarBG.y;

		healthBarFill.scrollFactor.set();
		healthBarFill.visible = !ClientPrefs.data.hideHud;
		healthBarFill.alpha = ClientPrefs.data.healthBarAlpha;
		healthBarFill.cameras = [camHUD];

		var playerColor = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
		var opponentColor = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);

		var shader = new shaders.ColorSwapeed();
		healthBarFill.shader = shader;
		shader.setColors(opponentColor, playerColor);
		shader.setDividePoint(0.5);

		uiGroup.add(healthBarFill);
		...
		healthBar = new Bar(0, healthBarBG.y + healthBarBG.height/2, 'healthBar', function() return health, 0, 2);
	...
	public function updateHealthBarFill() {
    	if (healthBarFill != null && healthBarFill.shader != null) {
        	var percent:Float = 1.0 - (Math.min(Math.max(health, 0), 2) / 2);
        	var shader:shaders.ColorSwapeed = cast healthBarFill.shader;
        	shader.setDividePoint(percent);
    	}
	}
